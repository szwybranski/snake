...
def move_snake():
    ...
    snake = [head] + snake
    if not make_snake_longer: del snake[-1]
...
while True:
    ...
    # collision detection 
    if wall_hit(snake[0]): sys.exit()
    if snake_hit_itself(snake): sys.exit()
    ...
    # make snake move faster as it grows
    time.sleep(1/(2*len(snake)))
